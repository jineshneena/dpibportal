...
<div class="row">
          @section('headtitle')
         <a href='{{ route("policyoverview",$policyId)}}'>{{$overviewData->policy_number }}</a> <i class="fas fa-angle-double-right"></i><span class='text-blue' style='font-size:25px'>{{$overviewData->endorsement_number}} </span>
@endsection
...
<div class="col-md-12 row button-group">
 ...
 <button type="button" class="btn btn-success btn-rounded dpib_endorsement_edit dib-cursor-style" editUrl='{!! route("editendorsement",[$policyId, $overviewData->id]) !!}' data-toggle="tooltip" title="" data-original-title="Edit endorsement"><i class="fas fa-edit"></i> Edit</button>                          
                            
                     
                                    </div>
    
    <div class="col-lg-3 col-xlg-3 mb-4" style='text-align:right'>
      @if ((in_array('ROLE_FINANCE_MANAGER', Auth::user()->roles) || in_array('ROLE_FINANCE', Auth::user()->roles) || in_array('ROLE_OPERATION_MANAGER', Auth::user()->roles)  ) && $overviewData->endorsement_status !=2)   
 @if($overviewData->endorsement_status==1)
     <button type="button" class="btn btn-success btn-rounded dpib_endorsement_issue dib-cursor-style" endorsement_id="{{$overviewData->id}}" policy_id="{{$policyId}}" data-toggle="tooltip" title="" data-original-title="Approve" ><i class="mdi mdi-thumb-up"></i> Approve</button>
     <button type="button" class="btn btn-success btn-rounded dib-cursor-style dpib_endorsement_reject" endorsement_id="{{$overviewData->id}}" policy_id="{{$policyId}}"  data-toggle="tooltip" title="" data-original-title="Reject" ><i class="mdi mdi-thumb-down"></i> Reject</button>
     @elseif((in_array('ROLE_FINANCE_MANAGER', Auth::user()->roles) || in_array('ROLE_FINANCE', Auth::user()->roles) || in_array('ROLE_OPERATION_MANAGER', Auth::user()->roles)  ) && $overviewData->endorsement_status==3)
     <button type="button" class="btn btn-success btn-rounded dib-cursor-style dpib_endorsement_issue"  endorsement_id="{{$overviewData->id}}" policy_id="{{$policyId}}" data-toggle="tooltip" title="" data-original-title="Approve" ><i class="mdi mdi-thumb-up"></i> Approve</button>
                        ...
                        <div id="customer_overview" class="panel-collapse panel-body ">
                            <table class="info-table" width='100%'>
                                <tbody>
                                    ...
                                    <tr><td>Amount:</td><td><b>{{ number_format($overviewData->amount,2)}} SAR</b></td></tr>
                                    <tr><td>Vat:</td><td><b>{{$overviewData->vat_percentage}}%</b></td></tr>
                                    <tr><td>Vat amount</td><td><b>{{number_format($overviewData->vat_amount,2)}} SAR</b></td></tr>
                                    <tr><td>Total amount</td><td><b>{{ number_format(($overviewData->vat_amount+$overviewData->amount),2)}} SAR</b></td></tr>
                ...
                <tbody><tr id="field_document_file" class="field"><td class=""><div class="label"><span class="text-danger icon-asterix"></span><span class="title">File</span></div></td><td><div class="element"><div><input type="file" id="document_file" name="document_file[]" multiple="multiple" onchange="FORM.validateFile($(this), 128, 'Maximum file upload size 128 MB exceeded!')"></div></div></td></tr>    <tr id="field_documenttype_oid" class="field ">
                        ...
                        <td>
                            <div class="element">
                             ...
                             {{ Form::select('documenttype_oid',[''=>'--- other ---'] +  $documentType, 11,array('id' =>'documenttype_oid','required'=>'required','class'=>'required form-control','error-message' =>"Gender field is mandatory" ))}}  
                            </div>
                        </td>
                    </tr>
        ...
        columnDefs1.push({"name": 'Edited at',  "targets": 5,  data: function (row, type, val, meta) {
         ...
         columnDefs1.push({"name": 'actions',  "targets": 6, "orderable": false, data: function (row, type, val, meta) {
                            ...
                            if(jQuery.inArray(fileExtension.toLowerCase(), extensionArray) !== -1){
                               var link ='<a href="{!!  url("/")  !!}/uploads/'+customerId+'/document/'+policyId+'/'+ fileName +'" class="dib-cursor-style"  target="_blank" style="margin-left:3px"><span class="fas fa-eye text-blue mr-right" data-toggle="tooltip" title="" data-original-title="Preview file" data-placement="left"></span></a>';
                              displayAction += link; 
                            }
