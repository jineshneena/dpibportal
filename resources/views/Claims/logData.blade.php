...
<script>
   ...
   $(function(){
        ...
        columnDefs.push({"name": 'createdby',  "targets": 0, data: function (row, type, val, meta) {
                            var subject =moment(row['entered_date']).format('DD.MM.YYYY HH:mm');
                            row.sortData = row['entered_date'];
        ...
        columnDefs.push({"name": 'oldValue',  "targets": 3, "orderable": false, data: function (row, type, val, meta) {
           ...
           columnDefs.push({"name": 'description',  "targets": 4, "orderable": false, data: function (row, type, val, meta) {
                            var subject = row['information'];
                            row.sortData = row['information'];
                            row.displayData = subject;
