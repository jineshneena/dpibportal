<div style="width:100px;position:relative;top:-38px;color:green"><a href="{{route('downloadtimeline',[$policyId])}}"><i class="fa fa-download" aria-hidden="true"></i></a></div>
<div class="timeline">
    @php
    $iCount = 1;
    @endphp
        ...
        <div class="timelinecontainer left">
            ...
            <div class="content">
                @if($details['type']==1)
                 <h2>Policy start</h2>
                <p>
                   Policy was issued on  {{date('d-m-Y', strtotime($details['issueDate']))}} with {{$details['installmentNumber']}} installment
                </p>
                @elseif($details['type']==2)
                 <h2>Installment{{$details['installment']}}</h2> 
                 <p>
                  Installment due amount is {{ number_format(($details['amount']+$details['vatAmount']), 2, '.',',')   }} SAR 
                 </p>
                ...
                <h2>Policy End</h2>
                        <p>
                    Policy ends on {{date('d-m-Y', strtotime($details['startDate']))}}
                            </p>
                @endif
