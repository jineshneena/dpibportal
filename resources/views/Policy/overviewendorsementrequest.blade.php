            ...
            <div class="col-md-12 row button-group">
                <div class="col-lg-6 col-xlg-6 mb-4" style='margin-left:26px'>
                 ...
                 @if($overviewData->type !=17)
                            ...
                            <button type="button" class="btn btn-success btn-rounded dpib_endorsement_crm_delete dib-cursor-style" delete_url="{!! route('deleteendorsementcrmrequest',$requestId) !!}" return-url="{!! route('dashboardendorsementlist') !!}" data-toggle="tooltip" title="" data-original-title="Delete request"><i class="fas fa-archive"></i> Delete</button>                          
                       @endif    
                ...
                <div class="col-lg-6 col-xlg-6 mb-4" style='text-align:right;margin-left:-118px'>
                 @if (in_array('ROLE_OPERATION_MANAGER', Auth::user()->roles) ||  Auth::user()->id == $overviewData->created_by || Auth::user()->id == $overviewData->assign_to)
                    {{ Form::open(array('route' => array('updateendorsementrequeststatus', $policyId), 'name' => 'form_crm_status_edit','id'=>'form_crm_status_edit','files'=>'true' )) }}
                                                      ...
                                                      @endforeach
                                                        ...
                                                        @endif
                                                      <input type='hidden' name='request_status' class="crm_status" value='{{$overviewData->status}}' />
                        ...
                        <table class="info-table" width='100%'>
                            <tbody>
                                ...
                                <tr><td>Policy no:</td><td><a href='{{ route("policyoverview",$policyId)}}'><b>{{$overviewData->policy_number }}</b></a></td></tr>
                                 ...
                                 @else
                                 <tr><td>Customer:</td><td><b>   <a href='{{ route('customeroverview',$overviewData->customer_id)}}'>{{ucfirst(trans($overviewData->customerName)) }}</a> </b></td></tr>
                                 @endif
                                ...
                                <tr>
                                    <td style='width:35%'>
                                        <span class='text-warning'> 
                                            ...
                                            @endif
                                        </span><br/><span  class="text-warning"> {{date('d.m.Y h:i:s A',strtotime($commentDetail-> created_at))}}</span>
                                    </td>
                                   
                                    <td><span style="margin-left:10px"><i>{{$commentDetail->comments}}</i></span></td>
...
<script id='announcement_document_upload_template' type='text/template'>
    {{ Form::open(array('route' => array('updateannouncementrequeststatus', $overviewData->id), 'name' => 'form_announcement_status','id'=>'form_announcement_status','files'=>'true' )) }}
