...
@section('headtitle')
<h3 class="text-blue bold">Customer report</h3>
@endsection
...
<div class="card">
    <div class="card-body">
        <h4 class="card-title">Filters</h4>
            ...
            {{ Form::open(array('route' => 'customerfilter', 'name' => 'customerfilter','id'=>'customerfilter','class'=>'') ) }}
            ...
            <div class="form-group row">
                <label for="example-email-input" class="col-2 col-form-label">Customer Type</label>
                <div class="col-10">
            ...
            <div class="form-group row">
                <label for="example-email-input" class="col-2 col-form-label">Customer group</label>
                <div class="col-10">
            ...
            <div class="form-group row">
                <label for="example-email-input" class="col-2 col-form-label">Account manager</label>
                <div class="col-10">
                     <input  class='form-control' name='accountmanager' id="accountmanager_name">
            ...
            <button type="button" class="btn btn-dark" id='filterCancel' redirectUrl='{{ route('salescustomer')}}'>Cancel</button>
...
<div class="card">
    <div class="card-body">
        <a href="{{ route('customerexport')}}"><button type="button" class="btn btn-info d-none d-lg-block m-l-15 pull-right"><i class="mdi mdi-file-export"></i> Export</button></a>
        ...
        <table class="table table-bordered table-striped dpib_quote_request_list color-table info-table" >
                    <thead>
                        <tr>
                            ...
                            <th  class="nowrap" style="width: 10%">Policy number</th>
                            <th  class="nowrap" style="width: 10%">Issue date</th>
                            <th  class="nowrap" style="width: 10%">End date</th>
...
$(function(){
                ...
                columnDefs.push({"name": 'policy number',  "targets": 6, data: function (row, type, val, meta) {
                ...
                columnDefs.push({"name": 'issue date',  "targets": 7, data: function (row, type, val, meta) {
                            
                            
                            row.sortData = moment(row['issue_date']).format("DD-MM-YYYY");
                            row.displayData = moment(row['issue_date']).format("DD-MM-YYYY") ;
                            
                            return row;
                        }, render: {"_": 'sortData', "display": 'displayData', 'filter': 'sortData'}});
                    
                columnDefs.push({"name": 'end date',  "targets": 8, data: function (row, type, val, meta) {
                         
                            
                            row.sortData =  moment(row['end_date']).format("DD-MM-YYYY");
                            row.displayData =  moment(row['end_date']).format("DD-MM-YYYY");
                            
                            return row;
                        }, render: {"_": 'sortData', "display": 'displayData', 'filter': 'sortData'}});
                    
                columnDefs.push({"name": 'createdat',  "targets": 9, "orderable": false, data: function (row, type, val, meta) {
  ...
  disabled: false,
   ...
   return false;
    ...
    $("#accountmanager_name").autocomplete({
  disabled: false,
   position: { my : "right top", at: "right bottom" },
      source: function( request, response ) {
        $.ajax( {
      ...
      minLength: 2,
      select: function( event, ui ) {
          $('#accountmanager_name').val(ui.item.label); // display the selected text
