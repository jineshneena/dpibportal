<?php $__env->startSection('createnewbutton'); ?>
...
<?php $__env->startSection('content'); ?>
                ...
                <?php if(in_array('ROLE_TECHNICAL_MANAGER', Auth::user()->roles) || in_array('ROLE_TECHNICAL', Auth::user()->roles)): ?> 
                            ...
                            <?php echo $__env->make('Dashboard.technicaldashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;
                  
                   
                     <?php elseif(in_array('ROLE_SALES_MANAGER', Auth::user()->roles) || in_array('ROLE_SALES', Auth::user()->roles)): ?> 

                            <?php echo $__env->make('Dashboard.salesdashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;
                            
                    <?php elseif(in_array('ROLE_OPERATION_MANAGER', Auth::user()->roles) || in_array('ROLE_OPERATION', Auth::user()->roles)): ?> 

                            <?php echo $__env->make('Dashboard.operationdashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;         
                            
                  <?php elseif(in_array('ROLE_FINANCE_MANAGER', Auth::user()->roles) || in_array('ROLE_FINANCE', Auth::user()->roles)): ?> 

                            <?php echo $__env->make('Dashboard.financedashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>;    
                      <?php endif; ?>                          
                        
                       <?php if( (in_array('ROLE_SALES_MANAGER', Auth::user()->roles) || in_array('ROLE_SALES', Auth::user()->roles)) && $taskFlag || in_array('ROLE_TECHNICAL_MANAGER', Auth::user()->roles) || in_array('ROLE_TECHNICAL', Auth::user()->roles) ): ?>
            <div class="row">
   <div class="col-lg-12">          
                       ...
                       <div class="card">
                          <div class="card-body">
                            ...
                            <table class="table table-bordered table-striped dpib_task_notification_table color-table info-table">
                                        ...
                                        <tbody>
                                             <?php if(count($taskDetails) > 0): ?>  
                                            <?php $__currentLoopData = $taskDetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $tasknotification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <tr class="tr-shadow">
                                                                                        
                                                <td><span class="block-email"><?php echo e($key+1); ?></span></td>
                                                <td class="desc"> <?php echo e(date('d.m.Y',strtotime($tasknotification->reminder_date))); ?> </td>
                                                <td class="desc"><?php echo e($tasknotification->description); ?></td>
                                                <?php
                                                $classValue='denied';
                                                ?>
                                                <?php if($tasknotification->priority ==2): ?>
                                                <?php
                                                $classValue='process';
                                                ?>
                                                                                               
                                                <?php endif; ?>
                                                <td class='<?php echo e($classValue); ?>'><?php echo e($tasknotification->priorityString); ?></td>
                                                
                                                
                                                <td><?php echo e($tasknotification->createdUser); ?></td>
                                                <td>
                                                    
                                                            <i class="ti-pencil task_reminder_edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" aria-describedby="tooltip404710" reminder_id='<?php echo e($tasknotification->id); ?>' saved-data="<?php echo e(json_encode($tasknotification)); ?>"></i>
                                                       &nbsp;&nbsp;
                                                  
                                                            <i class="ti-trash task_reminder_delete" data-placement="top" title="" data-original-title="Delete" remove_id='<?php echo e($tasknotification->id); ?>'></i>
                                                       </td>
...
<script id='task_notification_template' type='text/template'>
        ...
        <?php echo e(Form::open(array('route' => array('addtaskreminder'), 'name' => 'form_task_reminder_add','id'=>'form_task_reminder_add','files'=>'true' ))); ?>
                                    ...
                                    <div class="card-body card-block">
                                        <div class="form-group">
                                            <label for="company" class=" form-control-label">Date</label>
                                              ...
                                              <input type="date"  name="reminder_date" value="<% if(savedData.reminder_date){ %> <%- savedData.formattedate %>  <%} else {%> <?php echo e(date('d.m.Y')); ?> <% } %>"  id="testDatepicker" class="datefield1 form-control" >
                                        ...
                                        <div class="form-group">
                                            <label for="vat" class=" form-control-label">Priority</label>
                                                ...
                                                <?php echo e(Form::select('reminder_priority',  $priorityArray, null,array('id' =>'reminder_priority','required'=>'required','class'=>'form-control-lg form-control','error-message' =>"Gender field is mandatory" ))); ?>  
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <textarea  id="reminder_description" placeholder="Description" class="form-control" name="reminder_description"><% if(savedData.description){ %> <%- savedData.description %>  <%}%> </textarea>
                                                    <input type="hidden" name="reminder_type" value="<?php echo e($reminderType); ?>" />
                                        </div>
                                    ...
                                    <div class="card-body card-block">
                                        ...
                                        <div class="form-group">
                                          ...
                                          <div class="form-group" style="display:none">
                                            ...
                                            <?php echo e(Form::select('user_select', [''=>'---- select user ----'] +$userDetails, Auth::user()->id,array('id' =>'user_select','style'=>'width: 100%; ','class'=>"autocomplete form-control","required" =>"required" ,"error-message"=>'User field is mandatory'))); ?>
...
<script>
    ...
    var quoterequestTable = '';
      ...
      var graphData =<?php echo json_encode($dashboardDetails['graphData'], 15, 512) ?>; 
       <?php if(in_array('ROLE_SALES_MANAGER', Auth::user()->roles) || in_array('ROLE_SALES', Auth::user()->roles)): ?> 
   ...
   $(function(){
    ...
    $(document).on('change','.dpib_notification_action',function(){
        ...
        if($('.notofocation_change_id:checked').length > 0 && $('.dpib_notification_action').val() !='' ) {
                $.ajax({
                    url: '<?php echo route("changenotificationflag"); ?>',
                    type: "POST",
 ...
 /***    Task Notification    ***/   
    ...
    $(document).on('click','.task_reminder_delete',function(){
         $("#db_task_notification_deletepopup").remove();
                $('body').append('<div id="db_task_notification_deletepopup" title="Delete notification" class="col-lg-12" > <form id="form_delete_notification"  action=<?php echo route("deletenotification"); ?>>Do you want to delete reminder? <input type="hidden" name="notificationId" value="'+$(this).attr('remove_id')+'"/></form></div>');
    ...
    leadsTable =   $('.dpib_leads_table').DataTable({
                ajax : {
                            url: "<?php echo e(route('getcustomerdata',[$dashboardDetails['customerType']])); ?>",
                            type: "GET",
                            cache: false
                        },
                autoWidth: true,
...
<?php echo $__env->make('layouts.elite',['notificationCount'=>$notificationCount ] , \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Xampp_new\htdocs\inslyportal\resources\views/Dashboard/index.blade.php ENDPATH**/ ?>