...
<script>
        ...
        columnDefs.push({"name": 'Uploaded at',  "targets": 4,  data: function (row, type, val, meta) {
         ...
         columnDefs.push({"name": 'actions',  "targets": 5, "orderable": false, data: function (row, type, val, meta) {
                            ...
                            if(jQuery.inArray(fileExtension.toLowerCase(), extensionArray) !== -1){
                               var link ='<a href="<?php echo url("/"); ?>/uploads/'+row['customer_id']+'/document/'+ fileName +'" class="dib-cursor-style"  target="_blank" style="margin-left:3px"><span class="fas fa-eye text-blue mr-right" data-toggle="tooltip" title="" data-original-title="Preview file"></span></a>';
                              displayAction += link; 
                            }
   ...
   $('.dpib_customer_doc tbody').on( 'click', 'tr.group', function () {
    ...
    $(document).on('click','.dpib_customer_crm_document_add',function(){
        ...
        $.ajax({
            url: "<?php echo route('customercrmdocaddform',[$customerId,$crmId]); ?>",
            type: "GET"
    ...
    $(document).on('click','.dpib_document_delete',function(){
           $.ajax({
            url: "<?php echo route('customercrmdocdetaildelete',[$customerId,$crmId]); ?>",
            headers: {
    ...
    $(document).on('click','.dpib_document_edit',function(){
            ...
            var linkString = '<?php echo route("customercrmdocumenteditform",["##customerId##","##crmId##"]); ?>';
...
</script><?php /**PATH D:\Xampp_new\htdocs\inslyportal\resources\views/Request/crmDocumentData.blade.php ENDPATH**/ ?>