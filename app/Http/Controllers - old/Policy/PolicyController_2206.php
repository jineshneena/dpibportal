...
//dd($commissionDetails->toSql(),$commissionDetails->getBindings());
    ...
    public function changePolicyStatus(Request $request, $policyId) {
        ...
        if ($changedto == 4) {
            ...
            $maidetails['cc_data'] = [$technicalManager->email];
    ...
    public function addSalesPerson(Request $request, $policyId) {
        ...
        if ($request->get('person_sales_person') != '') {
            ...
            if ($request->get('type_sales_person') == 0) {
                ...
                $totalPremium = DB::table('policy_intallment')->where('policy_id', $policyId)->sum('amount');

                $policyCommission = DB::table('policies')->where('id', $policyId)->select('commision', 'additional_amount')->first();
                $additionalAmount = $policyCommission->additional_amount;
                $grossAmount = floatval($totalPremium - $additionalAmount);
                $companyCommision = floatval($grossAmount * ($policyCommission->commision / 100));

                $commissionSalesPerson = floatval($companyCommision * ($request->get('perc_sales_person') / 100));

                //echo $totalPremium."%%%%%". $policyCommission->commision."#####".$companyCommision."@@@@@".$commissionSalesPerson;exit;
                $insertArray[] = array("policy_id" => $policyId,
                    "distributor_type" => 'sales person',
                    "commission_type" => '0',
                    "percentage" => floatval($request->get('perc_sales_person')),
                    "amount" => $commissionSalesPerson,
                    "added_date" => $datetime,
                    "sales_person_id" => $request->get('person_sales_person'),
                );
            } else {
