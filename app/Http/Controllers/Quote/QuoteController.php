...
//        }
    ...
    public function savecrmrequestForm(Request $request, $customerId) {
        ...
        if ($request->has('activePolicyList') && $request->get('crm_type') == 3 && $request->has('activePolicyList') != '') {
            $crmMainObj->renewal_policy_id = $request->get('activePolicyList');
        }



        $crmMainObj->updated_date = date('Y-m-d h:i');
        $selectedcustomer = ($customerId <= 0) ? $request->get('customer_select'):$customerId;
        $crmMainObj->save();
...
//GET SALES MANAGER DETAILS
        ...
        //here file is upload and request assigned to technical department and status changed to technical review 
        if ($request->hasFile('document_file')) {
            $this->uploadRequestDoc($request,$selectedcustomer, $crmMainId);
            $url = route('crmrequestOverview', ['requestId' => $crmMainId]);
            $updatesArray['technical_reporting_date'] = date('Y-m-d h:i');
            $updatesArray['status'] = 2;
            //SEND MAIL TO TECHNICAL LEAD
            $userObj = App\User::where('id', Auth::user()->id)
                ->first();
            $users = DB::table('users')->select('email', 'name','id')->where('status', '1')->where('roles', 'like', "%ROLE_TECHNICAL_HEAD%")->orderBy('id')->first();
            $updatesArray['assigned_to'] = $users->id;
             DB::table('crm_main_table')->where('id',$crmMainId)->update($updatesArray);
            $data = array('name' => $userObj->name, 'link' => $url, 'Request_no' => $requestId, 'username' => $users->name);
...
//        }
    ...
    public function customerCrmStatusUpdate(Request $request, $customerId, $crmId) {
        ...
        if ($request->get('crm_status') == '5') {
            ...
            $maidetails['name'] = $users->name;
            $maidetails['subject'] = "CRM Request no: " . $requestNo . " status was changed";
            $created_user = DB::table('crm_main_table as cr')->join('users as u', 'cr.user_id', '=', 'u.id')->where('cr.id', $crmId)->pluck('u.email')->toArray();
           
            $combined_cc = array_merge($created_user,$cc_data);
            $maidetails['cc_data'] = $combined_cc;
            $this->send_email($maidetails, $data, $templatename);
...
//return view('comparisonExcel/sheet_1',  $requestArray);
     ...
     * @param type $crmId
     */
    private function uploadRequestDoc($request, $customerId, $crmId) {
        $files = $request->file('document_file');
        $insertArray = [];
        $type = 2;
        $comment = '';
