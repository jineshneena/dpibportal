...
class EndorsementController extends Controller {
    ...
    public function saveEndorsementDetails(Request $request, $policyId) {
        ...
        $overviewData = DB::table('crm_endorsement as ecr')                       
            ...
            // $this->sendMail($users, $overviewData ,'Verify insly entered flag of endorsement request:'.$overviewData->request_id);
             ...
             $financeuser = DB::table('users')->select('email', 'name')->where('status', '1')->where('roles', 'like', "%ROLE_FINANCE_ADMIN%")->orderBy('id')->first();
               ...
               //Update the status of crm request
        ...
        if ($oldStatus != 3) {
            $logArray = array('crm_id' => $crmId, 'kind' => 'Crm request status was changed to ' . $statusArray[3].' and endorsement details are added', 'old_value' => $statusArray[$oldStatus], 'updated_by' => Auth::user()->id, 'updated_at' => date('Y-m-d H:i:s'));
    ...
    public function saveEndorsementRequest(Request $request) {
        ...
        if (in_array('CUSTOMER_MANAGER', Auth::user()->roles) || in_array('CUSTOMER_OFFICER', Auth::user()->roles)) {
           return redirect()->route('overviewcustomerendorsementrequest', ['policyId' => $request->get('request_policy'), 'requestId' => $insertId])->with('success', 'Successfully created endorsement request!!!!');
        } else  { 
