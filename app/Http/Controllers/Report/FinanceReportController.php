...
class FinanceReportController extends Controller {
    ...
    public function productionExport() {
        ...
        $requestArray[] = array('Policy no:', 'Type','Product', 'Customer','Id code', 'Insurer','Inception date', 'End date','Due date','Issue date','Gross amount', 'Vat', 'Total amount', 'Commission', 'Commision amount','Commission vat','Unpaid','Collects','Sales person','Sales commission','Sales commission amount','Renewal status' );
        ...
        $AllListArray[] = array('Policy no:', 'Policy type','Product', 'Endorsement number','Type',  'Customer', 'Insurer','Inception date','End date', 'Due date','Issue date','Gross amount', 'Vat', 'Total amount', 'Commission', 'Commision amount','Commission vat','Unpaid','Collects','Sales person','Sales commission','Sales commission amount','Renewal status');
        ...
        foreach ($filteredResult as $result) {
            ...
            $requestArray [] = array(
                ...
                'Renewal status' => ($result->sales_type ==2) ? 'Renewal' :'New',
            );
        }
...
/**
 * Generate the report of the active policies
 */
    public function activePoliciesDetails() {

        $activePolicyDetails = DB::table('policies')
                        ->join('customers', 'customers.id', '=', 'policies.customer_id')
                        ->join('insurer_details', 'insurer_details.id', '=', 'policies.insurer_id')
                        ->leftJoin('customer_contact_person_info', 'customer_contact_person_info.customer_id', '=', 'customers.id')
                        ...
                        ->select('policies.*','customer_contact_person_info.person_name','customer_contact_person_info.email as contactEmail','customer_contact_person_info.mobile_phone as contactMob','customer_contact_person_info.phone as contactPhone',  DB::raw('DATE_FORMAT(policies.start_date, "%d.%m.%Y") as startDate'), DB::raw('DATE_FORMAT(policies.end_date, "%d.%m.%Y") as endDate'), 'insurer_details.insurer_name', 'policies.id as mainId', 'customers.name as customerName', 'insurance_product.product_name','customers.email as customerEmail','customers.phone as customerPhone','customers.mobile as customerMobile')
                       
                        ->orderBy('policies.issue_date', 'asc')->get();

        $requestArray[] = array('Policy no:', 'Customer', 'Insurer', 'Product', 'Customer phone', 'Customer email','Contact person','Email','Phone');

        foreach ($activePolicyDetails as $result) {
            $requestArray [] = array(
                'Policy no:' => $result->policy_number,
                'Customer' => $result->customerName,
                'Insurer' => $result->insurer_name,
                'Product' => $result->product_name,
                'Customer phone' => $result->customerPhone.",".$result->customerMobile,
                'Customer email' => $result->customerEmail,
                'Contact person' => $result->person_name,
                'Email' => $result->contactEmail,
                'Phone' => $result->contactMob.','.$result->contactPhone,
            );
        }



        Excel::create('activepolicydetails_' . date('Ymdhis'), function($excel) use ($requestArray) {
            $excel->setTitle('Post request details');
            $excel->sheet('Policies', function($sheet) use ($requestArray) {
                $sheet->fromArray($requestArray, null, 'A1', false, false);
                $sheet->setPageMargin(array(
